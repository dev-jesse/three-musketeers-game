...
public class Board extends Observable{
    ...
    private PieceCreator pieceCreator = new PieceCreator();
    /**
    ...
    public Board(File boardFile, String skinPath) {
    	this.skinPath = skinPath;
    	this.loadBoard(boardFile);
    }
    
    public PieceCreator getPieceCreator() {
    	return this.pieceCreator;
    }
    ...
    /**
     * @return the Type (Musketeer or Guard) of the current turn
     */
    public Type getTurn() {
        return turn;
    }
    ...
    public List<Cell> getMusketeerCells() {
        return getAllCells()
                .stream()
                .filter(cell -> cell.hasPiece() && cell.getPiece().getType() == Type.MUSKETEER)
                .toList();
    }
    ...
    public List<Cell> getGuardCells() {
        return getAllCells()
                .stream()
                .filter(cell -> cell.hasPiece() && cell.getPiece().getType() == Type.GUARD)
                .toList();
    }
    ...
    public List<Cell> getPossibleCells() {
        List<Cell> allCellsThisTurn = getTurn() == Type.MUSKETEER ? getMusketeerCells() : getGuardCells();
    ...
    private void changeTurn() {
        setTurn(getTurn() == Type.MUSKETEER ? Type.GUARD : Type.MUSKETEER);
    }
    
    protected void setSkin(String pathFile) {
    	this.skinPath = pathFile;
    	this.pieceCreator.setSkin(pathFile);
    }
    ...
    protected void loadBoard(File file) {
        ...
        turn = Type.valueOf(scanner.nextLine().toUpperCase());
        ...
        while (scanner.hasNextLine()) {
            ...
            for (String piece: pieces) {
                ...
                cell.setPiece(this.getPieceCreator().createPiece(piece));
    ...
    /**
     * 
     * Momento Interface + Momento for undo.
     * @return
     */
    public MomentoInterface save() {
    	...
    	return new ConcreteMomento(copy, this.turn, this.winner);
    }
    
    public void restore(MomentoInterface m) {
    	ConcreteMomento cm = (ConcreteMomento)m;
    	Cell[][] b = cm.getState();
    	
    	// board is final, cannot reassign, but can be cheeky and re-assign each cell.
    	for (int i = 0; i < this.size; i++){
    		for(int j = 0; j < this.size; j++) {
    			this.board[i][j].setPiece(b[i][j].getPiece());
    		}
    	}
    	setTurn(this.turn == Type.MUSKETEER ? Type.GUARD: Type.MUSKETEER);
      updateState();
    }

    public void updateState() {
    	BoardEvaluatorImpl i = new BoardEvaluatorImpl();
    	this.updateState(i.evaluateBoard(this));
    	notifyObserver();
    }
